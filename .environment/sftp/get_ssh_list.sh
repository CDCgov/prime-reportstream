...
SSHNAMES=$(az sshkey list --query "[?resourceGroup=='PRIME-DATA-HUB-$upper_env']" | jq -c '[.[] | select( .name | test("sftp.*-") ).name]')
...
jq -n --arg ssh "$SSH" --arg instances "$INSTANCES" --arg sshgroups "$SSHGROUPS" --arg sshnames "$SSHNAMES" --arg shares "$SHARES" '{"ssh":$ssh, "instances":$instances, "sshgroups":$sshgroups, "sshnames":$sshnames, "shares":$shares}'
