...
class WorkflowEngineTests {
    ...
    fun `test dispatchReport`() {
        ...
        val event = ReportEvent(EventAction.NONE, UUID.randomUUID(), false)
    ...
    fun `test dispatchReport with Error`() {
        ...
        val settings = FileSettings().loadOrganizations(oneOrganization)
        val report1 = Report(one, listOf(listOf("1", "2"), listOf("3", "4")), source = TestSource, metadata = metadata)
        val event = ReportEvent(EventAction.NONE, report1.id, false)
        val bodyFormat = MimeFormat.CSV
        val bodyUrl = "http://anyblob.com"
        val actionHistory = mockk<ActionHistory>()
        val receiver = Receiver("MyRcvr", "topic", Topic.TEST, CustomerStatus.INACTIVE, "mySchema")
...
// todo           blobMock.deleteBlob(blobUrl = any())
    ...
    fun `test handleReportEvent`() {
        ...
        val bodyFormat = "CSV"
        val bodyUrl = "http://anyblob.com"
        val event = ReportEvent(EventAction.SEND, report1.id, false)
        val nextAction = ReportEvent(EventAction.NONE, report1.id, false)
    ...
    fun `test handleProcessEvent queue vs task table mismatch error`() {
        ...
        val processEvent = ProcessEvent(EventAction.PROCESS, report1.id, ReportOptions.None, emptyMap(), emptyList())
        // Mismatch:  The event in the TASK table is a NONE event.
        val mismatchedNotProcessEvent = ReportEvent(EventAction.NONE, report1.id, false)
