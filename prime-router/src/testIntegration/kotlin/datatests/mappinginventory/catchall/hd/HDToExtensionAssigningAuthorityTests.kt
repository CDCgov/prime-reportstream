...
class HDToExtensionAssigningAuthorityTests {
    ...
    fun `test correctly handles UUID universal id type`() {
        assert(verifyHL7ToFHIRToHL7Mapping("hd/HD-to-extensionAssigningAuthority-uuid").passed)
    }

    @Test
    fun `test correctly handles unknown universal id type`() {
        assert(verifyHL7ToFHIRToHL7Mapping("hd/HD-to-extensionAssigningAuthority-clia").passed)
    }

    @Test
    fun `test correctly handles empty hd3`() {
        assert(verifyHL7ToFHIRToHL7Mapping("hd/HD-to-extensionAssigningAuthority-empty-hd3").passed)
    }
}