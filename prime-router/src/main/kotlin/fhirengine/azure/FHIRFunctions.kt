...
import gov.cdc.prime.router.fhirengine.engine.WithDownloadableReport
...
class FHIRFunctions(
     ...
     */
    private fun readMessage(engineType: String, message: String, dequeueCount: Int): WithDownloadableReport {
        logger.debug(
            "${StringUtils.removeEnd(engineType, "e")}ing message: $message for the $dequeueCount time"
        )

        return QueueMessage.deserialize(message) as WithDownloadableReport
    }
