...
abstract class HistoryDatabaseAccess(
    ...
    fun <T> fetchActionsForSubmissions(
        ...
        return db.transactReturning { txn ->
            val query = DSL.using(txn)
                ...
                .select(
                    Tables.ACTION.ACTION_ID,
                    Tables.ACTION.CREATED_AT,
                    Tables.ACTION.SENDING_ORG,
                    Tables.ACTION.SENDING_ORG_CLIENT,
                    Tables.REPORT_FILE.RECEIVING_ORG,
                    Tables.REPORT_FILE.RECEIVING_ORG_SVC,
                    Tables.ACTION.HTTP_STATUS,
                    Tables.ACTION.EXTERNAL_NAME,
                    ...
                    Tables.REPORT_FILE.BODY_FORMAT
                )
                .from(
    ...
    private fun createColumnSort(
        ...
        val column = when (sortColumn) {
            ...
            SortColumn.CREATED_AT -> Tables.ACTION.CREATED_AT
        }
    ...
    private fun createWhereCondition(
        ...
        if (since != null) {
            filter = filter.and(Tables.ACTION.CREATED_AT.ge(since))
        }

        if (until != null) {
            filter = filter.and(Tables.ACTION.CREATED_AT.lt(until))
        }
        ...
        return if (showFailed) {
            ...
            val failedFilter =
                Tables.ACTION.HTTP_STATUS.between(200, 299) // don't show failed = need to filter by status
