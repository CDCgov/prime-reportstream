...
abstract class CoolTest {
    ...
    suspend fun pollForProcessResult(
        ...
        var queryResults = emptyMap<ReportId, String?>()
        ...
        // all results should have a response, and all responses should be valid
        for (result in queryResults.values) {
            passed && result != null && examineProcessResponse(result)
        }
        return passed
    }
     ...
     * @return mutable map of responses, one per report generated by the process step
     */
    private fun queryForProcessResults(
        reportId: ReportId,
    ): MutableMap<ReportId, String?> {
        var queryResults = mutableMapOf<ReportId, String?>()
        db = WorkflowEngine().db
        db.transact { txn ->
    ...
    companion object {
         ...
         * @param internalReportId ReportID to find the first child process result of
        ...
        fun processActionResultQuery(
            ...
            val ctx = DSL.using(txn)
            // get report ids generated by the process step
            val processingReportIds = ctx.selectFrom(REPORT_LINEAGE)
                .where(REPORT_LINEAGE.PARENT_REPORT_ID.eq(internalReportId))
...
class End2End : CoolTest() {
    ...
    private suspend fun forceAsync(environment: Environment, options: CoolTestOptions): Boolean {
        ...
        if (reportId != null) {
            ...
            passed = passed and pollForProcessResult(internalReportId)
            if (!passed)
