...
class Settings : CoolTest() {
    ...
    private val apiPath = "/api/settings"
    ...
    override suspend fun run(environment: ReportStreamEnv, options: CoolTestOptions): Boolean {
        ...
        val testFileDir = File("./src/test/json_test_files")
        val (newName, newDummyOrganization) = SettingsUtilities.fromJson(
            SettingsUtilities.readInput(File(testFileDir, "DummyNew.json").inputStream()),
            SettingsUtilities.SettingType.ORG
        )
        echo("Read the NEW dummy organization json resource from: $testFileDir/$newName")
        ...
        if (!output.contains("PRIME's NEW Dummy Organization for Test CRUD of the Setting API")) {
        ...
        val (updateName, updateDummyOrganization) = SettingsUtilities.fromJson(
            SettingsUtilities.readInput(File(testFileDir, "DummyUpdate.json").inputStream()),
            SettingsUtilities.SettingType.ORG
        )
        echo("Read the UPDATE dummy organization json resource from: $testFileDir/$updateName")
        ...
        if (!output.contains("PRIME's UPDATED Dummy Organization for Test CRUD of the Setting API")) {
