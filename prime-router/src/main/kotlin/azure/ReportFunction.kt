...
class ReportFunction(
        ...
        val report = try {
            ...
            when (options) {
                Options.CheckConnections, Options.ValidatePayload -> {
                    ...
                    // if there are any errors, kick this out. If SkipInvalidItems was specified, duplicates
                    //  will be noted as 'warnings' instead of errors
    ...
    /**
     * Checks for duplicates by calculating the digest of the [rawBody] and checking the database to see if the
     * [sender] already has that digest. If it is a duplicate file, adds an error including [payloadName] if
     * provided. If entire file is not duplicated, checks the [report] rows looking for duplicates, and adds
     * either errors or warnings based on SkipInvalidItems in [options]
     */
    internal fun doDuplicateDetection(
        ...
        if (workflowEngine.isDuplicateFile(sender, digest)) {
            ...
            addDuplicateLogs(options, actionLogs, msg, null)
        } else {
            // item level row duplicate checking
            val removeList = mutableListOf<Int>()
            // keep list of hashes generated by this ingest to do same-file comparison
            val generatedHashes = mutableListOf<ByteArray>()
            for (rowNum in 0 until report.itemCount) {
                ...
                if (isDuplicate) {
                    ...
                    if (options == Options.SkipInvalidItems) {
                        removeList.add(rowNum)
                    }
                } else {
                    generatedHashes.add(itemHash)
                }
            }
            // if all items will be removed as duplicates, this is a completely duplicate file and we
            //  need to kick it out since we cannot have a report with 0 items
            if (removeList.size == report.itemCount) {
                var msg = "Duplicate file detected."
                if (!payloadName.isNullOrEmpty()) {
                    msg += "File: $payloadName"
                }
            ...
            // remove just duplicate rows from the report so we can still process the valid ones
                ...
                report.removeRowsFromTable(removeList)
            }
        }
    }

    /**
     * If a [rowNum] is passed, a DuplicateItemMessage with [message] is added to [actionLogs]
     * as either a warning or error, depending on if SkipInvalidItems is specified in [options].
    ...
    internal fun addDuplicateLogs(
        ...
        if (rowNum != null) {
            val msg = DuplicateItemMessage(message)

            if (options != Options.SkipInvalidItems)
                actionLogs.getItemLogger(rowNum)
                    .error(msg)
            else
                actionLogs.getItemLogger(rowNum)
                    .warn(msg)
            ...
            val msg = DuplicateFileMessage(message)
            actionLogs.error(msg)
        }
    }
