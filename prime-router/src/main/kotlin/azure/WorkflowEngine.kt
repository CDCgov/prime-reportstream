...
class WorkflowEngine(
    ...
    fun dispatchReport(
        ...
        val receivingApp: String? =
            if (isEmptyReport && receiver.translation is Hl7Configuration) {
    ...
    fun handleReportEvent(
        ...
        db.transact { txn ->
            ...
            val currentEventAction = IEvent.EventAction.parseQueueMessage(header.task.nextAction.literal)
    ...
    fun resendEvent(
        ...
        val nextEvent = ReportEvent(IEvent.EventAction.SEND, reportId, at = null, isEmptyBatch = false)
        db.transact { txn ->
            db.fetchAndLockTask(reportId, txn) // Required, it creates lock.
            val organization =
                settings.findOrganization(receiver.organizationName)
    ...
    fun generateEmptyReport(
        ...
        val toSchema =
            metadata.findSchema(receiver.schemaName)
        ...
        this.db.transact { txn ->
            val sendEvent = ReportEvent(IEvent.EventAction.SEND, emptyReport.id, true)
    ...
    private fun sendToDestination(
        ...
        when {
            options == Options.SkipSend -> {
                ...
                val event = ReportEvent(IEvent.EventAction.NONE, report.id, actionHistory.generatingEmptyReport)
            ...
            receiver.timing != null && options != Options.SendImmediately -> {
                ...
                val event = BatchEvent(IEvent.EventAction.BATCH, receiver.fullName, false, time)
            ...
            receiver.format.isSingleItemFormat -> {
                ...
                report
                    .split()
                    .forEach {
                        val event = ReportEvent(IEvent.EventAction.SEND, it.id, actionHistory.generatingEmptyReport)
                ...
                val event = ReportEvent(IEvent.EventAction.SEND, report.id, actionHistory.generatingEmptyReport)
                this.dispatchReport(event, report, actionHistory, receiver, txn)
    ...
    fun handleProcessEvent(
        ...
        db.transact { txn ->
            ...
            val currentAction = IEvent.EventAction.parseQueueMessage(task.nextAction.literal)
            if (currentAction != IEvent.EventAction.PROCESS) {
    ...
    fun handleBatchEvent(
        ...
        db.transact { txn ->
            val tasks =
                db.fetchAndLockBatchTasksForOneReceiver(
                    messageEvent.at,
            ...
            tasks.forEach {
                val currentAction = IEvent.EventAction.parseQueueMessage(it.nextAction.literal)
                updateHeader(
                    it.reportId,
                    currentAction,
                    TaskAction.none,
    ...
    private fun findOrganizationAndReceiver(
        ...
        if (settings is SettingsFacade) {
            val (organization, receiver) =
                (settings).findOrganizationAndReceiver(fullName, txn)
                    ?: error("Receiver not found in database: $fullName")
            Pair(organization, receiver)
        } else {
            val (organization, receiver) =
                settings.findOrganizationAndReceiver(fullName)
    ...
    private fun createHeader(
        ...
        val content =
            if (downloadContent && BlobAccess.exists(reportFile.bodyUrl)) {
    ...
    private fun updateHeader(
        reportId: ReportId,
        currentEventAction: IEvent.EventAction,
        nextTaskAction: TaskAction,
        nextActionAt: OffsetDateTime? = null,
        retryToken: String? = null,
        txn: DataAccessTransaction,
    ) {
        fun finishedField(currentEventAction: IEvent.EventAction): Field<OffsetDateTime> =
            when (currentEventAction) {
                IEvent.EventAction.RECEIVE -> Tables.TASK.TRANSLATED_AT
                IEvent.EventAction.PROCESS -> Tables.TASK.PROCESSED_AT
                ...
                IEvent.EventAction.CONVERT -> Tables.TASK.PROCESSED_AT
                IEvent.EventAction.ROUTE -> Tables.TASK.ROUTED_AT
                IEvent.EventAction.DESTINATION_FILTER -> Tables.TASK.DESTINATION_FILTERED_AT
                IEvent.EventAction.RECEIVER_FILTER -> Tables.TASK.RECEIVER_FILTERED_AT
                IEvent.EventAction.TRANSLATE -> Tables.TASK.TRANSLATED_AT
                IEvent.EventAction.REBATCH -> Tables.TASK.TRANSLATED_AT // overwrites prior date
                IEvent.EventAction.BATCH -> Tables.TASK.BATCHED_AT
                IEvent.EventAction.RESEND -> Tables.TASK.BATCHED_AT // overwrites prior date
                IEvent.EventAction.SEND -> Tables.TASK.SENT_AT
                IEvent.EventAction.WIPE -> Tables.TASK.WIPED_AT

                IEvent.EventAction.BATCH_ERROR,
                IEvent.EventAction.SEND_ERROR,
                IEvent.EventAction.PROCESS_ERROR,
                IEvent.EventAction.PROCESS_WARNING,
                IEvent.EventAction.WIPE_ERROR,
                -> Tables.TASK.ERRORED_AT

                IEvent.EventAction.NONE -> error("Internal Error: NONE currentAction")
                IEvent.EventAction.OTHER -> error("Internal Error: OTHER currentAction")
            }
