...
- `valueSet` - a list of key value pairs used to convert the value generated by the value
  property to another value that matches the key. An interface is provided to be able to
  extend this property with a custom class that returns the key value pairs programmatically;
  specify a named `type` to deserialize to a custom class. The UP implements these extended types:
  - `lookupValueset` - provide `tableName`, `keyColumn`, `valueColumn` to retrieve key value pairs from a lookup table.
