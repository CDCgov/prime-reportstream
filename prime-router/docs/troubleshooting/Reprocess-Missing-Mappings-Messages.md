...
## Introduction
To address semantic interoperability and data quality for receivers, ReportStream (RS) can filter out messages that contain the non-Reportable Condition Trigger Code (RCTC) condition code (e.g., LOINC or SNOMED CT).  During conversion of the incoming message to the internal FHIR format, RS compares LOINC or SNOMED codes from both OBX-3-1/Observation.code.coding.code (LOINC) and OBX-5-1/Observation.ValueCodeableConcept.coding.code (SNOMED) with values in the "code" column of the Observation Mapping table. If the value(s) are found in the table, it returns the value(s) from the "condition code" column. If there is no match, it logs the missing mapping error into the action_log table.

Values from the "Condition Code" and "Member OID" columns will then be appended to the internal FHIR bundle as elements that can be used in FHIRpath condition filter logic. For more details on the conversion process, please see the condition code mapping at [docs/design/proposals/0023-condition-to-code-mapping/0023-condition-to-code-mapping.md].
## Assumptions
* You have CDC credentials and have been granted access to the Azure portal. If you do not have credentials or access to the Azure portal, stop here. You cannot proceed.
...
1.	Log in to your Metabase production account. After logging in, the user should see the following screenshot and needs to click on the [+ New ] button as shown below:

![](./images/reprocess-missing-mapping-msg-pic-1.PNG)

2. The user should see the following screenshot. The user needs to click the SQL query option as shown below:

![](./images/reprocess-missing-mapping-msg-pic-2.PNG)

3. The user should see the following screenshot and copy the SQL into the Metabase input area as shown:
    ```SQL
...
4.	The user should see the following screenshot.  In the message column, the user should see the LOINC or SNOMED code.  In the body_url column, the user should see a link to the incoming message containing the code as shown below.
    ![](./images/reprocess-missing-mapping-msg-pic-4.PNG)
5. Investigate the code by examining the input file in the link. To examine the file, the user needs to use Microsoft Azure Storage Explorer and copy the file from the link into the Filter, as shown below.<br>
...
* Below is a screenshot of Microsoft Azure Storage Explorer showing how to download the file from the ‘pdhprodstorageaccount’ in Azure Blob Storage. The user needs to follow steps 1-4 as shown in the screenshot.

   ![](./images/reprocess-missing-mapping-msg-pic-6.PNG) 

6. To open the file with the editor

   ![](./images/reprocess-missing-mapping-msg-pic-7.PNG) 

## Update the missing code in the local system
1. When the user opens the message, the user  should see the screenshot below (I.e., Microsoft Visual Studio Code).
