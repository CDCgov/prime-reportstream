...
function docker_decompose() {
  ...
  for target in ${TARGETS[*]}; do
    ...
    docker compose --env-file .env --file "${target?}" down 2>/dev/null |
      tee -a "${LOG?}"
...
function ensure_build_dependencies() {
  ...
  docker compose --env-file .env --file "docker-compose.build.yml" up --detach "${BUILD_SERVICES[@]}" 1>>"${LOG?}" 2>&1
  if [[ ${?} != 0 ]]; then
...
function activate_containers() {
  ...
  docker compose --env-file .env --file "docker-compose.build.yml" up --detach postgresql 1>>"${LOG?}" 2>&1
  ...
  docker compose --env-file .env --file "docker-compose.yml" up --detach "${SERVICES[@]}" 1>>"${LOG?}" 2>&1
  ...
  if [[ "${OSTYPE?}" == "darwin"* ]] && [ $PROFILE = "amd64" ]; then
    ...
    sleep 5
    docker compose --env-file .env --file "docker-compose.yml" restart prime_dev 1>>"${LOG?}" 2>&1
  fi
...
function post_run_instructions() {
  if [ $PROFILE = "amd64" ]; then
      ...
      if [[ "${OSTYPE?}" == "darwin"* ]]; then
        ...
        echo "    \$ docker compose --env-file .env up --detach"
      fi
