...
docker compose --env-file .env -f ./docker-compose.build.yml ${STATE?} ${DETACH?} "${BUILD_SERVICES[@]}" 2>&1 |
  grep -v "${ORPHAN_CONTAINER_WARNING_MSG?}"

if [ "${PROFILE}" = "amd64" ]; then
  docker compose --env-file .env -f "./docker-compose.yml" build
fi
docker compose --env-file .env -f ./docker-compose.yml ${STATE?} ${DETACH} "${SERVICES[@]}" 2>&1 |
  grep -v "${ORPHAN_CONTAINER_WARNING_MSG?}" |
