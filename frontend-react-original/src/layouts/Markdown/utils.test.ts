...
describe("createMeta", () => {
    ...
    test("merges overlapping areas", () => {
        const meta = createMeta(
            configOverlapFixture as any,
            frontmatterOverlapFixture as any,
        );
        expect(meta.openGraph.image.src).toBe(
            configOverlapFixture.PAGE_META.defaults.openGraph.image.src,
        );
        expect(meta.openGraph.image.altText).toBe(
            frontmatterOverlapFixture.meta.openGraph.image.altText,
        );
    });
    test("doesn't merge empty strings", () => {
        const meta = createMeta(
            configOverlapFixture as any,
            frontmatterOverlapEmptyFixture as any,
        );
        expect(meta.openGraph.image.altText).toBe(
            configOverlapFixture.PAGE_META.defaults.openGraph.image.altText,
        );
    });
});
