...
export interface TableFilterSearchData {
    resultLength?: number;
...
function TableFilterSearch({ resetHandler, searchReset, setSearchTerm, resetFilterFields }: TableFilterSearchProps) {
    ...
    return (
        <div className="margin-bottom-4 padding-left-4">
            <label id="search-field-label" data-testid="label" className="usa-label" htmlFor="search-field">
                Search by Report ID or Filename
            </label>
            <div className="usa-hint margin-bottom-105">
                Enter full Report ID or Filename, including file extension when applicable.
            </div>
            <div className="display-flex file-search-container">
                <Search key={searchReset} onSubmit={submitHandler} className="margin-right-205 height-5" />
                <Button onClick={resetHandler} type={"reset"} name="clear-button" unstyled>
                    Reset
                </Button>
            </div>
        </div>
