...
describe("MessageReceivers component", () => {
    test("renders expected content", () => {
        ...
        expect(within(screen.getAllByRole("row")[1]).getAllByRole("cell")[0]).toHaveTextContent("md-phd");

        fireEvent.click(screen.getByText(/^Report Id$/));

        expect(within(screen.getAllByRole("row")[1]).getAllByRole("cell")[0]).toHaveTextContent("ak-phd");
    });
});
