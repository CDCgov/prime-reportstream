...
describe("ReceiversStatusesDisplay", () => {
    test("renders", async () => {
        const { baseElement } = renderApp(
            <ReceiversStatusesDisplay receiverStatuses={mockReceiversStatusesTimePeriod} />,
        );
        ...
        const clickableSlices = mockReceiversStatusesTimePeriod.reduce((prev, curr) => {
            return (
                prev +
        ...
        // Testing Library's findAllByRole does not support filtering by aria-disabled value,
        // so querySelectorAll is the only way to select enabled buttons specifically.
        // eslint-disable-next-line testing-library/no-node-access
        const clickableSliceElements = baseElement.querySelectorAll(`[role="button"][aria-disabled="false"]`);
