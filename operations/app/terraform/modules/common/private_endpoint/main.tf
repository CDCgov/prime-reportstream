...
locals {
  suitable_subnets = length(var.exclude_subnets) == 0 ? var.endpoint_subnet_ids : setsubtract(var.endpoint_subnet_ids, var.exclude_subnets)
}

resource "azurerm_private_endpoint" "endpoint" {
  count = length(local.suitable_subnets)

  name                = "${var.name}-${var.type}-${substr(sha1(local.suitable_subnets[count.index]), 0, 9)}"
  location            = var.location
  resource_group_name = var.resource_group
  subnet_id           = local.suitable_subnets[count.index]
