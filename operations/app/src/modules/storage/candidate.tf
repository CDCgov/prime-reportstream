...
# Grant the storage account Key Vault access, to access encryption keys
resource "azurerm_key_vault_access_policy" "storage_partner_candidate_policy" {
  key_vault_id = var.key_vault_id
  tenant_id = data.azurerm_client_config.current.tenant_id
...
resource "azurerm_storage_account_customer_managed_key" "storage_partner_candidate_key" {
  count = var.rsa_key_4096 != null && var.rsa_key_4096 != "" ? 1 : 0
  key_name = var.rsa_key_4096
  key_vault_id = var.key_vault_id
  key_version = null // Null allows automatic key rotation
  ...
  depends_on = [azurerm_key_vault_access_policy.storage_partner_candidate_policy]
}
...
resource "azurerm_storage_container" "storage_container_candidate_hhsprotect" {
  name = "hhsprotect"
  storage_account_name = azurerm_storage_account.storage_partner_candidate.name
}

resource "azurerm_storage_management_policy" "storage_partner_candidate_retention_policy" {
...
output "storage_partner_candidate_connection_string" {
  value = azurerm_storage_account.storage_partner_candidate.primary_connection_string
  sensitive = true
}