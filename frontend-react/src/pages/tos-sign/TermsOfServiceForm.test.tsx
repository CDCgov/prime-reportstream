...
describe('Basic rendering', () => {
    ...
    test('Required fields remove error once field is filled in and re-submitted', () => {
        ...
        fireEvent.change(emailInput, { target: { value: site.orgs.CDC.email } })
        fireEvent.change(organizationInput, { target: { value: 'Foobar' } })
        fireEvent.click(agreedInput)
        fireEvent.click(button)

        expect(screen.queryByAltText('First name is a required field')).toBeFalsy()
        expect(screen.queryByAltText('Last name is a required field')).toBeFalsy()
        expect(screen.queryByAltText('Email is a required field')).toBeFalsy()
        expect(screen.queryByAltText('Organization is a required field')).toBeFalsy()
        expect(screen.queryByAltText('You must agree to the Terms of Service before using ReportStream')).toBeFalsy()
    })
})
