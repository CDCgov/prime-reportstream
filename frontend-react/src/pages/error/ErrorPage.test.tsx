...
import { GENERIC_ERROR_PAGE_CONFIG, GENERIC_ERROR_STRING } from "../../content/error/ErrorMessages";
...
describe("ErrorPage tests", () => {
    ...
    test('Renders as page with type="page" prop', () => {
        ...
        expect(screen.getByRole("heading")).toHaveTextContent("An error has occurred");
    });
    ...
    test("Including type (old prop) and config (new prop) will result in NEW prop being used", () => {
        renderApp(<ErrorPage type={"message"} config={GENERIC_ERROR_PAGE_CONFIG} />);
        expect(screen.getByRole("heading")).toHaveTextContent("An error has occurred");
    });
});
