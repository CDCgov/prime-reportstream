...
const dataToAccordionItems = (props: { id: string; title: string; data: WarningError[] }): AccordionItemProps[] => {
    ...
    results.push({
        ...
        title: props.title,
        expanded: false,
        content: <WarningsErrors title={""} data={props.data.map((md) => md.detail)} />,
        headingLevel: "h3",
        className: "",
    });
...
export function MessageDetails() {
    ...
    const submittedDate = messageDetails?.submittedDate ? new Date(messageDetails.submittedDate) : undefined;
        ...
        <GridContainer>
            <article className="margin-bottom-10" data-testid="container">
                <div>
                    ...
                    <Button onClick={() => navigate("/admin/message-tracker")} type="button">
                        Back
                    </Button>
                </div>
                <hr />
                <div className="display-flex flex-column">
                    <span className="text-base font-sans-sm margin-bottom-1">Message ID</span>
                ...
                <div className="display-flex flex-column margin-bottom-7">
                    ...
                    <div className="font-sans-sm bg-gray-5 radius-lg padding-top-3 padding-bottom-2 padding-left-3 padding-right-3">
                        <div className={warnings.length || errors.length ? "margin-bottom-4" : ""}>
                            <DetailItem item="Submitter" content={messageDetails!.sender} />
                            <DetailItem item="Incoming Report ID" content={messageDetails!.reportId} />
                            <DetailItem
                                item="Date/Time Submitted"
                                content={submittedDate ? new Date(submittedDate).toLocaleString() : ""}
                            />
                            <div className="display-flex flex-column margin-bottom-2">
                                <span className="text-base line-height-body-5">{"File Location"}</span>
                                <div>
                            ...
                            <DetailItem item="Incoming File Name" content={fileName} />
                        </div>

                        <div
                            className={
                                warnings.length ? "message-tracker display-flex flex-column margin-bottom-2" : ""
                            }
                        >
                            <Accordion
                                bordered={true}
                                ...
                                items={dataToAccordionItems({
                                    ...
                                    title: `Warnings (${messageDetails!.warnings.length})`,
                                    data: warnings,
                                })}
                            />
                        </div>
                        ...
                        <div
                            className={errors.length ? "message-tracker display-flex flex-column margin-bottom-2" : ""}
                        >
                            <Accordion
                                bordered={true}
                                multiselectable={true}
                                items={dataToAccordionItems({
                                    ...
                                    title: `Errors (${messageDetails!.errors.length})`,
                                    data: errors,
                                })}
                            />
                        </div>
                ...
                {messageDetails?.receiverData ? (
                    <MessageReceivers receiverDetails={messageDetails.receiverData} />
                ) : (
                    <>
                        <h2>Receivers:</h2>
...
export const MessageDetailsPage = () => withCatchAndSuspense(<MessageDetails />);
