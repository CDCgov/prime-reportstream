...
import { type RSSessionContext, staticAuthorizedFetch } from "../SessionProvider";
import { configFixture, contextFixture } from "../useSessionContext.fixtures";

const useSessionContext = vi.fn(() => contextFixture);

export function mockSessionContentReturnValue(impl?: Partial<RSSessionContext>): () => RSSessionContext {
    const newValue = {
    ...
    if (!impl?.authorizedFetch) {
        newValue.authorizedFetch = (options: Partial<AxiosOptionsWithSegments>, endpointConfig?: RSEndpoint) => {
