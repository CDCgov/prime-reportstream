...
import { LookupTable, LookupTables, lookupTablesEndpoints } from "../../../../config/endpoints/lookupTables";
 ...
 */
const findTableMetaByName = (tableName: string, tables: LookupTable[] = []): LookupTable | undefined => {
    if (!tables.length) {
        return undefined;
    }
    const filteredBody: LookupTable[] = tables.filter((tv: LookupTable) => tv.tableName === tableName && tv.isActive);

    if (!filteredBody.length) {
        return undefined;
    }
    return filteredBody.sort((a: LookupTable, b: LookupTable) => b.tableVersion - a.tableVersion)[0];
};
...
const useValueSetsMeta = (dataTableName: string = LookupTables.VALUE_SET) => {
    ...
    const tableMeta = useMemo(() => findTableMetaByName(dataTableName, tableData), [dataTableName, tableData]);

    useEffect(() => {
        if (!tableMeta || tableData?.length) {
            rsConsole.info("Unable to find metadata for lookup table: ", dataTableName);
        }
