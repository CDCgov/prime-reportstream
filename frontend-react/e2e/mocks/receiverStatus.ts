...
export function createMockGetReceiverStatusSet(
    {range, maxMinutesPerStatus = 2 * 60, receiver, statusType = "mix", randomlySkip, statusIdStart = 1}: {range?: [start: Date, end: Date],
    maxMinutesPerStatus?: number, statusType?: "success" | "fail" | "mix", randomlySkip?: boolean, statusIdStart?: number, receiver: {organizationName: string, organizationId: number, receiverName: string, receiverId: number}}
) {
    if(range) {
    ...
    const failLength = statusType === "fail" ? statusSetLength : statusType === "mix" ? randomInt(3, Math.ceil(statusSetLength / 3)) : 0;
...
export function createMockGetReceiverStatus(range?: [start: Date, end: Date], maxMinutesPerStatus?: number) {
  ...
  const skipped = [{...orgA, receiverId: 5, receiverName: "skipped"} as unknown as AdmConnStatusDataType]

  return [success, fail, mixedWithSkips, skipped].flat();
}
