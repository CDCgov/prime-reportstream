...
test.describe("Submission history submissionHistoryPage.page", () => {
    ...
    test.describe("admin user", () => {
        ...
        test.describe("without org selected", () => {
            test("will not load submissionHistoryPage.page", async ({
                submissionHistoryPage,
            }) => {
                await expect(
                    submissionHistoryPage.page.getByText(
            ...
            test("has footer", async ({ submissionHistoryPage }) => {
                await expect(
                    submissionHistoryPage.page.locator("footer"),
                ).toBeAttached();
            });
        });
        ...
        test.describe("with org selected", () => {
            ...
            test("has filter", async ({ submissionHistoryPage }) => {
                await expect(
                    submissionHistoryPage.page.getByTestId("filter-container"),
                ).toBeAttached();
            });

            test.describe("table", () => {
                test("table has correct headers", async ({
                    submissionHistoryPage,
                }) => {
                    await submissionHistory.tableHeaders(
                ...
                test("table column 'ReportId' will open the report details", async ({
                    ...
                    const reportId = tableRows(submissionHistoryPage.page)
                        .nth(0)
                        .locator("td")
                        .nth(0);
                ...
                test("table has pagination", async ({
                    submissionHistoryPage,
                }) => {
                    await expect(
                        submissionHistoryPage.page.getByTestId(
    ...
    test.describe("sender user", () => {
        ...
        test.describe("table", () => {
            ...
            test("table column 'Date/time submitted' has expected data", async ({
                ...
                expect(
                    await tableDataCellValue(submissionHistoryPage.page, 0, 1),
                ).toEqual("3/7/2024, 6:00:22 PM");
            });
            ...
            test("table column 'Records' has expected data", async ({
                ...
                expect(
                    await tableDataCellValue(submissionHistoryPage.page, 0, 3),
                ).toEqual("1");
            });
            ...
            test("table column 'Status' has expected data", async ({
                ...
                expect(
                    await tableDataCellValue(submissionHistoryPage.page, 0, 4),
                ).toEqual("Success");
            });
